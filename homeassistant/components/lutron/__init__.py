...
async def async_setup_entry(hass, entry):
    ...
    LutronDevice.include_areas(
        entry.options.get(CONF_ENABLE_AREAS, DEFAULT_ENABLE_AREAS)
    )
    ...
    for area in hass_data[LUTRON_CONTROLLER].areas:
        ...
        for output in area.outputs:
            if output.type == "SYSTEM_SHADE":
                hass_data[LUTRON_DEVICES]["cover"].append((suggested_area, output))
            elif output.is_dimmable:
                hass_data[LUTRON_DEVICES]["light"].append((suggested_area, output))
            else:
                hass_data[LUTRON_DEVICES]["switch"].append((suggested_area, output))
            ...
            for button in keypad.buttons:
                ...
                if button.name != "Unknown Button" and button.button_type in (
                    ...
                    hass_data[LUTRON_DEVICES]["scene"].append(
                        (suggested_area, keypad, button, led)
                    )

                hass_button = LutronButton(hass, suggested_area, keypad, button)
        ...
        if area.occupancy_group is not None and area.sensors:
            hass_data[LUTRON_DEVICES]["binary_sensor"].append(
                (suggested_area, area.occupancy_group)
            )
...
class LutronDevice(Entity):
    ...
    _include_areas = False

    @classmethod
    def include_areas(cls, enabled):
        """Include Lutron Area information."""
        cls._include_areas = enabled
...
class LutronButton:
    ...
    def __init__(self, hass, area, keypad, button):
        ...
        self._full_id = slugify(f"{area} {keypad.name}: {button.name}")
